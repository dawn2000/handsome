...
class AdminSetting{
    ...
    public static function checkupdatejs()
    {
        ...
        $root = base64_decode(Config::version);

        $root_use = base64_decode(Config::debug);
        $html = <<<EOF
<script>
...
<script src="{$themeUrl}assets/js/jquery.min.js" type="text/javascript"></script>    
<script src="{$themeUrl}assets/js/admin/admin.min.js" type="text/javascript"></script>  
EOF;
...
//        $version = (string)self::version;
        ...
        if (Handsome::getPluginVersion("Handsome_Plugin", "Handsome") !== Handsome::version){
            echo '<script>alert("Handsome 插件未安装或未更新到最新版本，更新后请重新禁用再启用Handsome插件一次")</script>';
        }else{
            if (!Handsome::getPluginCorrect()){
                echo '<script>alert("Handsome 插件需要先禁用一次再重新启用以便重新注册相关钩子")</script>';
            }
        }
